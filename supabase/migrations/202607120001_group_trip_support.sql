-- 1. Add `registered` boolean to customers
--    All existing customers are private-chat users, so backfill to true
ALTER TABLE public.customers
  ADD COLUMN IF NOT EXISTS registered boolean NOT NULL DEFAULT false;

UPDATE public.customers SET registered = true;

-- 2. Modify match_active_trips RPC to include registered status
--    and order by registered DESC (registered drivers first)
