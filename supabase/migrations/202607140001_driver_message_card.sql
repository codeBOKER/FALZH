-- Add driver message card support to driver_trips

ALTER TABLE driver_trips
  ADD COLUMN driver_message text,
  ADD COLUMN use_driver_message boolean NOT NULL DEFAULT false;

-- Update match_active_trips to include driver message fields
...
AS $$
  WITH ranked AS (
    SELECT
      ...
      driver_trips.driver_message,
      driver_trips.use_driver_message,
  ...
  SELECT
    ...
    ranked.registered,
    ranked.driver_message,
    ranked.use_driver_message
  FROM ranked
